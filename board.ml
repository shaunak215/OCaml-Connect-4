...
let insert_piece (col: int) (brd: t): t =
  ...
  let get_row (row: int) (b: player list list): player list =
    ...
    List.fold b ~init:[] ~f:(fun init l -> 
      match List.nth l row with
      | Some p -> p :: init
      | None -> Empty :: init
      )

  let get_right_diagonal (row: int) (col: int) (b: player list list): player list =
    let rec iterate (r: int) (c: int) (diag_list: player list) (b: player list list): player list =
  ...
  let get_left_diagonal (row: int) (col: int) (b: player list list): player list =
    let rec iterate (r: int) (c: int) (diag_list: player list) (b: player list list): player list =
      match List.nth b c with
      | None -> diag_list
      | Some l -> 
  ...
  let connect_four (plist: player list): bool =
    let check_list = List.group plist ~break:(fun p1 p2 -> 
      match p1, p2 with
      | P1, P1 -> false
      | P2, P2 -> false
      | _, _ -> true
    ...
    if List.length has_four = 0 then false
    else true
  ...
  let is_win (col: int) (b: player list list): bool =
    ...
    if connect_four col_list || connect_four row_list || 
      connect_four rdiag_list || connect_four ldiag_list then true
    else false
